...
int numero=0;
float decimal=0;
int direccion=0,direccion_val=0;;
float  derivativo=0, proporcional=0, integral=0; //parametros
int  salida_pwm=0;
...
int divisor_f=0;
ISR(TIMER2_OVF_vect)//@16.384
{
	divisor_f++;
	if (divisor_f==10)
	{
		if (salida_pwm<0){velocidad_d_o=velocidad_d*-1;velocidad_i_o=velocidad_i*-1;}
		if (salida_pwm>0){velocidad_d_o=velocidad_d;velocidad_i_o=velocidad_i;}
	//Serial.print(velocidad_d_o);Serial.print("|||");Serial.print(rev_d);
	//Serial.print("&&&"); Serial.print(velocidad_i_o);Serial.print("|||");Serial.println(rev_i);
	//digitalWrite(13, !digitalRead(13));
	velocidad_d=0;
	velocidad_i=0;
	divisor_f=0;
...
attachInterrupt(0, cambios, RISING);// interrupcion de flanco de subida pin2
attachInterrupt(1, cambios2, RISING);// interrupcion de flanco de subida pin3
 //funcoones de interrupcion 
...
void loop()
{
  ...
  while( bluetooth.available())
  {
    ...
    if(c=='#')//bandera set point
    {
     // Serial.print("dato set point: ");
   ...
   if(c=='p')//bandera kp
    {
      //decimal=readString.toFloat();
      //Serial.print("dato const kp ");
       //Serial.println(readString);
    }     
...
//////////////////////////////////////////////////////////
float ANGULO_Y=angle_x+60;
int ANGULO_y=ANGULO_Y*100;/////////////////////////////////////////////////
 if(ANGULO_y>12000)ANGULO_y=12000;
 if(ANGULO_y<0)ANGULO_y=0;
 

int position = ANGULO_y;
//Serial.println(gyro_angle_z);
	...
	//negra, 1 para linea blanca
        ...
        int velocidad=200;
	if(position>6200){velocidad=200; }
        
        if(position<5800){velocidad=200;}
      	
        if(direccion==0)   direccion_val=0;
        if(direccion==1)  direccion_val=-300;
        if(direccion==2)  direccion_val=300;
        if(direccion==3)  direccion_val=-150;
        if(direccion==4)  direccion_val=-150;
		///if (velocidad_i_o>2){correccion=velocidad_i_o*3;}
		//if (velocidad_i_o<-2){correccion=velocidad_i_o*3;}
		//	if (velocidad_i_o>-20 && velocidad_i_o<20){correccion=-0;}
        proporcional = (position+numero)+(direccion_val)+correccion -6000; // set point es 3500, asi obtenemos el error
	integral=integral + proporcional*dt; //obteniendo integral
	derivativo = (proporcional - proporcional_pasado)/dt; //obteniedo el derivativo
	if (integral>555) integral=555; //limitamos la integral para no cusar problemas
	if (integral<-555) integral=-555;
...
void motor_i(int motor_izq)
{
	...
	{
		digitalWrite(8,HIGH); // con high avanza
		analogWrite(10,255-motor_izq); //se controla de manera
		//inversa para mayor control
	}
	else
	{
		digitalWrite(8,LOW); //con low retrocede
		motor_izq = motor_izq*(-1); //cambio de signo
		analogWrite(10,motor_izq);
	}
...
{
	if ( motor_der>= 0 )  //motor izquierdo
	{
		digitalWrite(7,HIGH); // con high avanza
		analogWrite(9,255-motor_der); //se controla de manera
		//inversa para mayor control
	}
	else
	{
		digitalWrite(7,LOW); //con low retrocede
		motor_der = motor_der*(-1); //cambio de signo
		analogWrite(9,motor_der);
	}
}
